...
type
  ...
  TPDFOption = (poOutLine, poCompressText, poCompressFonts, poCompressImages, poUseRawJPEG, poNoEmbeddedFonts, poPageOriginAtTop, poSubsetFont, poMetadataEntry, poNoTrailerID, poPDFA);
...
begin
  ...
  CreateInfoEntry;
  if (poMetadataEntry in Options) or (poPDFA in Options) then
    CreateMetadataEntry;
  if (not (poNoTrailerID in Options)) or (poPDFA in Options) then
    CreateTrailerID;
  CreatePreferencesEntry;
  if poPDFA in Options then
    AddPDFA1sRGBOutputIntent;
